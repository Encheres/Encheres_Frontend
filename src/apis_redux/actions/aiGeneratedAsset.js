...
export const fetchAiGeneratedLiteratureAsset = (seed_text, next_words_count) => async (dispatch, getState) => {

	dispatch({ type: GENERATED_ASSET_PROCESSING})
	try {
		let response = await fetch(
			ai_model_api_url + `/literature-generation?seed_text=${seed_text}&next_words_count=${next_words_count}`, 
			{
				method: "GET",
				headers: { "Content-Type": "application/json" },
			}

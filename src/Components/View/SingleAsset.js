...
class SingleAssetDetail extends React.Component {
    ...
    async submitBid(event_end_date_time) {
        ...
        if (this.state.bid <= this.state.price) {
            this.setState({
                bidsError:
                    "You can only Place a Bid higher than current Price!!",
            });
            return;
        } else {
            this.setState({
    ...
    async onBuyRequest(event_end_date_time) {
        ...
        if (this.props.physicalAsset.isBidLegal) {
            swal(
                "Success!!",
                "Congratulations!! You just bought a Physical Asset Move to Winnings Section Now.",
                "success"
            );
        } else {
    ...
    render() {
        if (this.props.items.isLoading) {
            ...
            return <RenderError error={this.props.items.errMess} />;
        } else {
                            ...
                            <Card id="new-item-card">
                                <CardBody>
                                    ...
                                    <CardSubtitle
                                        ...
                                        id="new-item-card-info"
                                    >
                                        {moment(asset.event_end_date_time).diff(
                                            moment(new Date())
                                        ) > 0 ? (
                                            <div>
                                                Sale Ends on{" "}
                                                {moment(
                                                    asset.event_end_date_time
                                                ).format(
                                                    "MMMM Do YYYY, h:mm A"
                                                )}
                                            </div>
                                        ...
                                        >
                                            ...
                                            >
                                                {asset.pickup_point
                                                    .addressLine1 +
                                                    ", " +
                                                    asset.pickup_point.city +
                                                    ", " +
                                                    asset.pickup_point.state +
                                                    ", " +
                                                    asset.pickup_point.country}
                                            </p>
                                        </CardSubtitle>
                                    </div>
                                    ...
                                    <div
                                        className="mt-4"
                                        id="single-asset-purchase-button-container"
                                    >
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        UpdatePhysicalAsset: (assetId, updatedAsset) =>
            dispatch(UpdatePhysicalAsset(assetId, updatedAsset)),
    };
};

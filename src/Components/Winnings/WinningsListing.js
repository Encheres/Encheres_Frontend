...
class Winnings extends Component {
    ...
    async loadAccountSmartContract() {
        ...
        if (networkData) {
            const account_contract = new web3.eth.Contract(
                Account.abi,
                networkData.address
            );
    ...
    async onSubmitPayment(assetId, receiver, payer) {
        ...
        let paymentAmount = window.web3.utils.toWei(
            this.state.totalPayment.toString(),
            "Ether"
        );

        await this.state.account_smart_contract.methods
            .receivePayment(receiver)
            ...
            swal({
                ...
                text:
                    this.props.orders.postFailMess || this.props.items.errMess
                        ? this.props.orders.postFailMess ||
    ...
    renderTransaction = (asset) => {
        ...
        if (!asset.sale) {
            if (asset.shipped) {
                transactionButton = (
                    <Button id="single-asset-purchase-button" disabled>
                        Asset Shipped
                    </Button>
                );
            } else {
                        ...
                        <Button
                            ...
                            onClick={() =>
                                this.onSubmitPayment(
                                    asset._id,
                                    asset.owner.accounts[0],
                                    asset.bidder.accounts[0]
                                )
                            }
                        >
            ...
            <Card className="col-11" id="winning-card">
                <div className="row">
                    <div className="col-12 col-md-12 col-lg-3">
                        <h6 className="winning-card-label rainbow-lr">
                            Asset ID
                        </h6>
                    ...
                    <div className="col-12 col-md-12 col-lg-3">
                        ...
                        <Link
                            className="winning-card-link"
                            to={`/view/independent-physical-assets/${asset._id}`}
                        >
                            <h6 className="winning-card-detail">
                                @{asset.asset.name}
                            </h6>
                        </Link>
                    </div>
                ...
                <Modal isOpen={this.state.paymentModal}>
                    <ModalHeader
                        style={{ backgroundColor: "#0B1126", borderWidth: 0 }}
                    >
                        <div style={{ color: "grey" }}>Enter Total Payment</div>
                    </ModalHeader>
                    <ModalBody
                        style={{ backgroundColor: "#0B1126", borderWidth: 0 }}
                    >
                        <div className="mb-4" id="new-item-form-error">
                            {
                                "Payment Can't be reverted ! Fill Carefully and then submit."
                            }
                        </div>
                        ...
                        <div className="mb-4" id="new-item-form-error">
                            {this.state.errors.totalPayment}
                        </div>
                    </ModalBody>
                    <ModalFooter
                        style={{ backgroundColor: "#0B1126", borderWidth: 0 }}
    ...
    render() {
        if (this.props.winnings.isLoading && !this.state.winnings.length) {
            ...
            return <RenderError error={this.props.winnings.errMess} />;
        } else {
            return (
                <div>
                    ...
                    {this.state.winnings.length ? (
                        <InfiniteScroll
                            ...
                            hasMore={
                                this.props.winnings.winnings.length
                                    ? true
                                    : false
                            }
                            loader={
                        ...
                        >
                            {this.state.winnings.map((asset) =>
                                this.renderTransaction(asset)
                            )}
                        </InfiniteScroll>
                    ) : (
                        <div>
